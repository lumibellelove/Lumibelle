(() => {
  ...
  const mails = [
    {
      ...
      title: "첫 루미 방문을 기억해요",
      meta: "루미벨 · 안내",
      desc: "루미벨과 처음 만난 날을 기념하는 샘플 우편이에요.",
      body: "처음 루미벨을 만나러 와준 날을 루미폰에 조용히 기록해둘게요. 지금은 샘플 우편이지만, 나중에는 공연 후 멤버의 한마디나 안내 우편이 이곳에 도착해요.",
      note: "받은 우편에서 열람 후 소장 우편으로 보관하는 흐름을 연결할 예정이에요."
    },
    {
      id: "mail-debut-live",
      box: "받은 우편",
      ...
      meta: "2026.07.12 · 라이브",
      desc: "공연 입장 기록과 함께 보관되는 안내 우편 샘플이에요.",
      body: "2026.07.12 데뷔 라이브에 함께한 기록이에요. 공연, 티켓, 체크인, 스탬프 기록은 추억의 시간과 함께 천천히 연결됩니다.",
      note: "실제 운영에서는 예매/입장/체크인 기록과 연결해 표시할 수 있어요."
    },
    {
      id: "mail-keep-sample",
      box: "소장 우편",
    ...
    },
    {
      ...
      note: "Stage 24에서는 상세보기와 소장 우편 안내까지만 1차 연결했어요."
    }
  ];
  ...
  function renderMail() {
    ...
    if (!visible.length) {
      ...
      list.innerHTML = visible.map((mail) => `
        <button type="button" class="mail-split-card" data-mail-id="${escapeAttr(mail.id)}">
          <span class="mail-split-icon">${mail.icon}</span>
          <span class="mail-split-meta">${mail.meta}</span>
          <b>${mail.title}</b>
          <span class="mail-split-chip">${mail.cat}</span>
  ...
  function openMail(id) {
    ...
    if (note) note.textContent = mail.note;
    if (keep) keep.textContent = mail.box === "소장 우편" ? "소장 우편에 보관됨" : "소장 우편으로 보관";
