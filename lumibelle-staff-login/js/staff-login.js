(function () {
  ...
  function setLoginMode(nextMode) {
    ...
    if (pinLoginSection) {
      pinLoginSection.hidden = isPasswordMode;
    }

    if (loginModeDivider) {
      loginModeDivider.hidden = isPasswordMode;
    }

    if (passwordPanel) {
      passwordPanel.hidden = !isPasswordMode;
    }

    if (passwordToggle) {
      passwordToggle.setAttribute('aria-expanded', String(isPasswordMode));
    }

    if (toggleText) {
      toggleText.textContent = isPasswordMode ? 'PIN 빠른 로그인으로 돌아가기' : '비밀번호로 로그인';
    }

    if (toggleArrow) {
      toggleArrow.textContent = isPasswordMode ? '‹' : '›';
    }
