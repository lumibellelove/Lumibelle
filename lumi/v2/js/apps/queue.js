...
var QueueStore = (function () {
  ...
  function normalizeData(data) {
    ...
    // `pausedMembers` only pauses the next staff call in an active lane.
    // Fan-registration availability is intentionally stored separately.
    if (!Array.isArray(data.registrationPausedMembers)) data.registrationPausedMembers = [];
    ...
    // v36: 사전 접수는 특전회 시작 전까지 유지한다. 이전 자동 마감용 값은 호환 정리만 한다.
    if (typeof data.registration.preregistrationDeadlineAt === "undefined") data.registration.preregistrationDeadlineAt = null;
    if (typeof data.registration.preregEndedAt === "undefined") data.registration.preregEndedAt = null;
    ...
    data.queues.forEach(function (row) {
      ...
      if (typeof row.deferCount === "undefined") row.deferCount = 0;
      if (typeof row.deferredAt === "undefined") row.deferredAt = null;
      if (typeof row.deferredFromStatus === "undefined") row.deferredFromStatus = null;
      if (!Array.isArray(row.deferHistory)) row.deferHistory = [];
      if (typeof row.registrationSource === "undefined") row.registrationSource = "";
    });
    normalizeQueueOrders(data);
    return data;
  }
  ...
  function compareQueueOrder(a, b) {
    var orderA = Number(a && a.queueOrder);
    var orderB = Number(b && b.queueOrder);
    var hasOrderA = isFinite(orderA) && orderA > 0;
    var hasOrderB = isFinite(orderB) && orderB > 0;

    if (hasOrderA && hasOrderB && orderA !== orderB) return orderA - orderB;
    if (hasOrderA && !hasOrderB) return -1;
    if (!hasOrderA && hasOrderB) return 1;

    var numberA = Number(a && a.number) || 0;
    var numberB = Number(b && b.number) || 0;
    return numberA - numberB;
  }

  function normalizeQueueOrders(data) {
    if (!data || !Array.isArray(data.queues)) return false;

    var changed = false;
    var rows = data.queues.slice().sort(compareQueueOrder);
    var used = {};
    var lastOrder = 0;

    rows.forEach(function (row) {
      var current = Number(row.queueOrder);
      var valid = isFinite(current) && current > 0 && !used[current];
      if (!valid) {
        current = Math.max(lastOrder + 1, Number(row.number) || 1);
        while (used[current]) current += 1;
        row.queueOrder = current;
        changed = true;
      }
      used[current] = true;
      lastOrder = Math.max(lastOrder, current);
    });

    return changed;
  }

  function nextQueueOrder(data) {
    var max = 0;
    (data.queues || []).forEach(function (row) {
      max = Math.max(max, Number(row.queueOrder) || 0);
    });
    return max + 1;
  }

  function activeQueueStatus(status) {
    return status === "대기중" || status === "호출중" || status === "진행중";
  }

  function findQueueRow(data, identifier) {
    var key = String(identifier == null ? "" : identifier);
    return (data.queues || []).find(function (row) {
      return String(row.id) === key ||
        String(row.number) === key ||
        String(row.queueId || "") === key;
    }) || null;
  }

  function normalizedLumiId(value) {
    return String(value || "").trim().toUpperCase();
  }

  function memberNameFromInput(value) {
    var raw = String(value || "").trim();
    if (MEMBERS.indexOf(raw) !== -1) return raw;
    var mapping = {
      mariring: "마리링",
      lulu: "루루",
      iro: "이로",
  ...
  function memberIdFromName(value) {
    var raw = String(value || "").trim();
    var mapping = {
      "마리링": "mariring",
      "루루": "lulu",
  ...
  function isRegistrationPausedForMember(data, member) {
    return Array.isArray(data.registrationPausedMembers) &&
      data.registrationPausedMembers.indexOf(member) !== -1;
  }

  function queueRowsForMember(data, member) {
    return (data.queues || []).filter(function (row) {
      return row.member === member && activeQueueStatus(row.status);
    }).sort(compareQueueOrder);
  }

  function hasTrailingActiveQueue(data, row) {
    if (!row) return false;
    var rowOrder = Number(row.queueOrder) || 0;
    return queueRowsForMember(data, row.member).some(function (candidate) {
      return candidate.id !== row.id && (Number(candidate.queueOrder) || 0) > rowOrder;
    });
  }

  function estimateMinutesForQueue(data, row) {
    if (!row) return 0;
    var rowOrder = Number(row.queueOrder) || 0;
    var minutes = queueRowsForMember(data, row.member)
      .filter(function (candidate) {
        return candidate.id !== row.id && (Number(candidate.queueOrder) || 0) < rowOrder;
      })
      .reduce(function (sum, candidate) {
        return sum + Math.max(0, Number(candidate.estimateMin || 0));
      }, 0);

    return Math.max(0, Math.round(minutes));
  }

  function estimateMinutesForNewRegistration(data, member) {
    var minutes = queueRowsForMember(data, member).reduce(function (sum, candidate) {
      return sum + Math.max(0, Number(candidate.estimateMin || 0));
    }, 0);
    return Math.max(0, Math.round(minutes));
  }
  ...
  function formatEstimate(minutes) {
    return minutes > 0 ? minutes + "분" : "대기 없음";
  }

  function fanQueueStatus(row) {
    if (!row) return "waiting";
    if (row.status === "대기중") return "waiting";
    if (row.status === "진행중") return "in-progress";
    if (row.status === "완료") return "completed";
    if (row.status === "취소") return "cancelled";
    if (row.status === "호출중") {
      if (row.needsResponseCheck) return "response-check";
      return Number(row.callRound || 1) >= 2 ? "second-call" : "first-call";
    }
    return "waiting";
  }

  function fanBenefitId(row) {
    if (!row) return "two-shot";
    if (row.item === "투샷 체키") return "two-shot";
    if (row.item === "핀체키") return "pin-cheki";
    if (row.item === "숙제체키") return "homework";
    if (row.item === "영상권") return "video";
    if (row.item === "샤메") return "same";
    if (row.item === "이벤트 특전권") return "event";
    if (row.item === "단체 촬영" && row.shootFormat === "체키") return "group-cheki";
    if (row.item === "단체 촬영" && row.shootFormat === "샤메") return "group-same";
    return "two-shot";
  }

  function fanBenefitPayload(benefitId) {
    var mapping = {
      "two-shot": { item: "투샷 체키", shootFormat: "", estimateMin: 10 },
      "pin-cheki": { item: "핀체키", shootFormat: "", estimateMin: 8 },
      "homework": { item: "숙제체키", shootFormat: "", estimateMin: 8 },
      "video": { item: "영상권", shootFormat: "", estimateMin: 10 },
      "same": { item: "샤메", shootFormat: "", estimateMin: 8 },
      "event": { item: "이벤트 특전권", shootFormat: "", estimateMin: 8 },
      "group-cheki": { item: "단체 촬영", shootFormat: "체키", estimateMin: 12 },
      "group-same": { item: "단체 촬영", shootFormat: "샤메", estimateMin: 12 }
    };
    return mapping[String(benefitId || "")] || null;
  }

  function findActiveFanQueue(data, lumiId) {
    var key = normalizedLumiId(lumiId);
    if (!key) return null;
    return (data.queues || []).filter(function (row) {
      return normalizedLumiId(row.lumiId) === key && activeQueueStatus(row.status);
    }).sort(compareQueueOrder)[0] || null;
  }

  function toFanQueueRecord(data, row) {
    if (!row) return null;
    var status = fanQueueStatus(row);
    var trailing = hasTrailingActiveQueue(data, row);

    return {
      queueId: row.id,
      eventId: (data.event || {}).eventId || "",
      memberId: memberIdFromName(row.member),
      benefitId: fanBenefitId(row),
      displayNumber: String(row.number || "—").padStart(3, "0") + "번",
      status: status,
      callRound: Number(row.callRound || 0),
      needsResponseCheck: Boolean(row.needsResponseCheck),
      acceptedAt: row.registeredAt || null,
      estimatedWait: formatEstimate(estimateMinutesForQueue(data, row)),
      aheadCount: queueRowsForMember(data, row.member).filter(function (candidate) {
        return candidate.id !== row.id && (Number(candidate.queueOrder) || 0) < (Number(row.queueOrder) || 0);
      }).length,
      deferCount: Number(row.deferCount || 0),
      deferredAt: row.deferredAt || null,
      canDefer: (status === "waiting" || status === "first-call" || status === "second-call") &&
        Number(row.deferCount || 0) < 1 &&
        trailing,
      hasTrailingQueue: trailing
    };
  }

  function fanSnapshot(data) {
    var registration = data.registration || {};
    var phase = data.started ? "closed" : (registration.phase || "auto_wait");

    return {
      event: {
        eventId: (data.event || {}).eventId || "",
        eventTitle: (data.event || {}).title || "Lumibelle Debut Live",
        description: "원하는 멤버 특전 대기를 신청할 수 있어요.",
        registrationPhase: phase,
        staffRegistrationPhase: registration.phase || "auto_wait",
        laneCount: MEMBERS.length,
        totalWaiting: (data.queues || []).filter(function (row) { return row.status === "대기중"; }).length
      },
      members: MEMBERS.map(function (member) {
        var current = currentCall(data, member);
        var waitingRows = (data.queues || []).filter(function (row) {
          return row.member === member && row.status === "대기중";
        }).sort(compareQueueOrder);
        var open = phase === "open" && !isRegistrationPausedForMember(data, member);
        var currentLabel = current
          ? "현재 " + String(current.number || "").padStart(3, "0") + "번"
          : (waitingRows.length ? "현재 " + String(waitingRows[0].number || "").padStart(3, "0") + "번 대기" : "현재 대기 없음");

        return {
          id: memberIdFromName(member),
          member: member,
          name: member,
          current: currentLabel,
          waiting: "대기 " + waitingRows.length + "명",
          waitingCount: waitingRows.length,
          line: (MEMBERS.indexOf(member) + 1) + " 레인",
          estimatedWait: formatEstimate(estimateMinutesForNewRegistration(data, member)),
          open: open,
          registrationPaused: !open
        };
      })
    };
  }

  function validateFanEvent(data, eventId) {
    var currentEventId = String((data.event || {}).eventId || "");
    if (eventId && currentEventId && String(eventId) !== currentEventId) {
      return "현재 특전회 정보와 일치하지 않습니다.";
    }
    return "";
  }

  function fanState(input) {
    var data = read();
    var request = input || {};
    var eventError = validateFanEvent(data, request.eventId);
    if (eventError) return { ok: false, message: eventError, data: data };

    var fan = request.fan || request;
    var lumiId = normalizedLumiId(fan.lumiId || request.lumiId || request.fanId);

    return {
      ok: true,
      snapshot: fanSnapshot(data),
      activeQueue: toFanQueueRecord(data, findActiveFanQueue(data, lumiId))
    };
  }
  ...
  function ensureTicketTypeTestSamples(data) {
    ...
    samples.forEach(function (sample) {
      ...
      data.queues.push({
        ...
        queueOrder: Number(sample.number),
  ...
  function migratePreRegistrationDeadlineLogic(data) {
    if (!data || !data.registration) return false;
    var registration = data.registration;
    var changed = false;

    // v33~v35의 자동 마감 실험 데이터를 정리한다.
    // 사전 접수는 특전회 시작 전까지 유지되고, 시작 이후에는 기존 현장 접수 엔진으로 이어진다.
    if (registration.preregistrationDeadlineAt) {
      registration.preregistrationDeadlineAt = null;
      changed = true;
    }
    if (!data.started && registration.phase === "closed") {
      registration.phase = "open";
      registration.closedAt = null;
      registration.overrideReason = "legacy_deadline_removed";
      appendAction(data, "PREREGISTRATION_DEADLINE_REMOVED", {});
      changed = true;
    }
    if (data.started && registration.phase !== "onsite") {
      registration.phase = "onsite";
      registration.preregEndedAt = registration.preregEndedAt || data.event.startedAt || new Date().toISOString();
      registration.closedAt = null;
      changed = true;
    }
    if (Number(registration.schemaVersion || 0) < 2) {
  ...
  function syncRegistrationSchedule(data) {
    ...
    if (!registrationAutoOpenDue(data)) return false;

    var registration = data.registration;
    registration.phase = "open";
    registration.openedAt = new Date().toISOString();
    registration.closedAt = null;
    registration.preregEndedAt = null;
    registration.overrideReason = "automatic";
    appendAction(data, "REGISTRATION_OPENED_AUTOMATICALLY", {});
    return true;
  }
  ...
  function read() {
    ...
    if (migratePreRegistrationDeadlineLogic(data)) changed = true;
  ...
  function start() {
    ...
    // 사전 접수 단계만 종료한다. 이후 신규 등록은 기존 현장 접수 엔진이 호출 마감 전까지 계속 받는다.
    data.registration.phase = "onsite";
    data.registration.preregEndedAt = data.event.startedAt;
    data.registration.closedAt = null;
    data.registration.autoOpenEnabled = false;
    appendAction(data, "PREREGISTRATION_ENDED_BY_SPECIAL_START", {});
  ...
  function addRegistration(rowInput) {
    ...
    // 특전회 시작 전에는 사전 접수 상태가 열려 있어야 하고,
    // 시작 후에는 기존 현장 접수 엔진이 호출 마감 전까지 같은 대기열에 계속 추가한다.
    if (!data.started && (!data.registration || data.registration.phase !== "open")) {
      return { ok: false, message: "현재 사전 접수가 열려 있지 않습니다.", data: data };
    }

    var input = rowInput || {};
    var source = input.source || input.registrationSource || "staff";
    var member = memberNameFromInput(input.member || input.memberId);
    var lumiId = normalizedLumiId(input.lumiId || input.fanId);

    if (MEMBERS.indexOf(member) === -1) return { ok: false, message: "멤버를 확인하지 못했습니다.", data: data };
    if (source === "fan" && isRegistrationPausedForMember(data, member)) {
      return { ok: false, message: "현재 해당 멤버의 접수가 일시 정지되었어요.", data: data };
    }
    if (source === "fan" && !lumiId) {
      return { ok: false, message: "팬 계정 정보를 확인하지 못했습니다.", data: data };
    }

    // 팬은 한 공연에서 활성 대기 1건만 유지한다.
    // 번호는 접수 기록으로 고정하고, 실제 줄 순서는 queueOrder로 따로 관리한다.
    if (source === "fan" && findActiveFanQueue(data, lumiId)) {
      return { ok: false, code: "ACTIVE_QUEUE_EXISTS", message: "이미 대기 중인 특전이 있어요.", data: data };
    }
    ...
    var row = {
      ...
      queueOrder: nextQueueOrder(data),
      ...
      registrationStage: data.started ? "onsite" : "preregistration",
  ...
  function defer(identifier, source) {
    var data = read();
    var row = findQueueRow(data, identifier);
    var requestSource = source || "fan";

    if (!row) return { ok: false, message: "순서를 미룰 대기 정보를 찾지 못했습니다.", data: data };
    if (row.status !== "대기중" && row.status !== "호출중") {
      return { ok: false, message: "현재 상태에서는 순서를 미룰 수 없습니다.", data: data };
    }
    if (row.needsResponseCheck) {
      return { ok: false, message: "현장 확인 중에는 순서를 미룰 수 없습니다.", data: data };
    }
    if (Number(row.deferCount || 0) >= 1) {
      return { ok: false, message: "순서 미루기는 대기 1건당 1회만 가능해요.", data: data };
    }
    if (!hasTrailingActiveQueue(data, row)) {
      return { ok: false, message: "같은 멤버 대기열에 뒤로 보낼 대기자가 없어요.", data: data };
    }

    var previousStatus = row.status;
    var previousOrder = Number(row.queueOrder) || 0;
    row.queueOrder = nextQueueOrder(data);
    row.status = "대기중";
    row.callRound = 0;
    row.calledAt = null;
    row.warning30At = null;
    ...
    row.deferredFromStatus = previousStatus;
    row.deferCount = Number(row.deferCount || 0) + 1;
    row.deferHistory = Array.isArray(row.deferHistory) ? row.deferHistory : [];
    row.deferHistory.unshift({
      at: row.deferredAt,
      source: requestSource,
      previousStatus: previousStatus,
      previousQueueOrder: previousOrder,
      queueOrder: row.queueOrder
    });

    appendAction(data, "QUEUE_DEFERRED_BY_FAN", {
      number: row.number,
      member: row.member,
      source: requestSource,
      previousStatus: previousStatus,
    ...
    // If the deferred fan was the current call, the next person is immediately
    // 1차 호출되어 line flow does not stop. Waiting rows simply move to the tail.
    var next = callFirstIfEligible(data, row.member, "FAN_DEFER");
    if (!write(data)) return { ok: false, message: "브라우저 저장에 실패했습니다.", data: data };
    return { ok: true, data: data, row: row, next: next };
  }

  function cancelByFan(identifier, reason) {
    var data = read();
    var row = findQueueRow(data, identifier);

    if (!row) return { ok: false, message: "취소할 대기 정보를 찾지 못했습니다.", data: data };
    if (row.status !== "대기중" && row.status !== "호출중") {
      return { ok: false, message: "현재 상태에서는 대기를 취소할 수 없습니다.", data: data };
    }

    row.status = "취소";
    row.cancelledAt = new Date().toISOString();
    row.cancelReason = reason || "팬 대기 취소";
    row.cancelType = "팬 취소";
    row.fanNotice = "팬 요청으로 대기 접수가 취소되었어요.";
    row.callRound = 0;
    row.calledAt = null;
    row.warning30At = null;
    row.needsResponseCheck = false;
    ...
    appendAction(data, "QUEUE_CANCELLED_BY_FAN", {
      number: row.number,
      member: row.member,
      reason: row.cancelReason
    });

    // Called rows must be replaced immediately. For a waiting row this only
    // fills a missing current call when the lane is eligible.
    var next = callFirstIfEligible(data, row.member, "FAN_CANCEL");
    if (!write(data)) return { ok: false, message: "브라우저 저장에 실패했습니다.", data: data };
    return { ok: true, data: data, cancelled: row, next: next };
  }

  function setRegistrationPaused(memberInput, paused) {
    var data = read();
    var member = memberNameFromInput(memberInput);
    if (MEMBERS.indexOf(member) === -1) return { ok: false, message: "멤버를 확인하지 못했습니다.", data: data };

    var list = data.registrationPausedMembers;
    var index = list.indexOf(member);
    if (paused && index === -1) list.push(member);
    if (!paused && index !== -1) list.splice(index, 1);

    appendAction(data, paused ? "FAN_REGISTRATION_MEMBER_PAUSED" : "FAN_REGISTRATION_MEMBER_RESUMED", {
      member: member
    });
    if (!write(data)) return { ok: false, message: "브라우저 저장에 실패했습니다.", data: data };
    return { ok: true, data: data, member: member, paused: Boolean(paused) };
  }

  function fanRegister(input) {
    var data = read();
    var request = input || {};
    var eventError = validateFanEvent(data, request.eventId);
    if (eventError) return { ok: false, message: eventError, data: data };

    var benefit = fanBenefitPayload(request.benefitId);
    if (!benefit) return { ok: false, message: "특전 정보를 확인하지 못했습니다.", data: data };

    var fan = request.fan || request;
    var result = addRegistration({
      source: "fan",
      lumiId: fan.lumiId || request.lumiId || request.fanId,
      displayName: fan.displayName || request.displayName,
      member: request.member || request.memberId,
      item: benefit.item,
      shootFormat: benefit.shootFormat,
      quantity: request.quantity || 1,
      estimateMin: benefit.estimateMin
    });

    if (!result.ok) return result;
    return {
      ok: true,
      snapshot: fanSnapshot(result.data),
      activeQueue: toFanQueueRecord(result.data, result.row)
    };
  }

  function fanRefresh(input) {
    return fanState(input);
  }

  function fanDefer(input) {
    var data = read();
    var request = input || {};
    var eventError = validateFanEvent(data, request.eventId);
    if (eventError) return { ok: false, message: eventError, data: data };
    ...
    var fan = request.fan || request;
    var lumiId = normalizedLumiId(fan.lumiId || request.lumiId || request.fanId);
    var row = findQueueRow(data, request.queueId || request.number) || findActiveFanQueue(data, lumiId);

    if (!row || normalizedLumiId(row.lumiId) !== lumiId) {
      return { ok: false, message: "내 대기 정보를 확인하지 못했습니다.", data: data };
    }

    var result = defer(row.id, "fan");
    if (!result.ok) return result;
    return {
      ok: true,
      snapshot: fanSnapshot(result.data),
  ...
  function fanCancel(input) {
    ...
    var result = cancelByFan(row.id, "팬 대기 취소");
    if (!result.ok) return result;
    return {
      ok: true,
      snapshot: fanSnapshot(result.data),
  ...
  function updateRegistration(action) {
    ...
    if (action === "open") {
      ...
      registration.preregistrationDeadlineAt = null;
      registration.preregEndedAt = null;
      appendAction(data, "REGISTRATION_OPENED_MANUALLY", {});
    } else if (action === "hold") {
  ...
  function setAutoOpenTime(value) {
    ...
    appendAction(data, "REGISTRATION_AUTO_OPEN_TIME_CHANGED", { autoOpenAt: value });
    if (!write(data)) return { ok: false, message: "브라우저 저장에 실패했습니다.", data: data };
  ...
  return {
    ...
    saveMemo: saveMemo,
    setPaused: setPaused,
    // Fan-only processing engine. These methods intentionally have no staff UI binding.
    defer: defer,
    cancelByFan: cancelByFan,
    setRegistrationPaused: setRegistrationPaused,
    getFanState: fanState,
    refreshFanState: fanRefresh,
    registerFromFan: fanRegister,
    deferFromFan: fanDefer,
    cancelFromFan: fanCancel,
...
function getQueueOverviewStateCopy(phase) {
  if (phase === "open") {
    return {
      ...
      note: '사전 접수 중 · 특전회 시작 후에는 현장 접수로 계속 이어집니다.',
      buttonLabel: '특전회 시작'
    };
  }
  if (phase === "paused") {
    return {
      ...
      helper: 'ⓘ 접수를 다시 열어야 특전회를 시작할 수 있습니다.',
...
function renderQueueWaitingOverview(data) {
  ...
  var startAllowed = phase === "open" ? "" : " disabled";
  var startLabel = phase === "open" ? "특전회 시작" : overviewState.buttonLabel;
...
function renderQueuePrestart(data, showExceptionControls) {
  ...
  var settingNote = phase === "open"
    ? '특전회 시작 전까지 팬 사전 접수가 진행됩니다.'
    : (phase === "paused" ? '현재 신규 접수가 일시정지되어 있습니다.'
      : (phase === "held" ? '자동 오픈이 보류되어 있습니다.'
        : (phase === "closed" ? '사전 접수 상태를 확인해주세요.'
...
function renderRegistrationExceptionControls(phase) {
  ...
  return '<section class="queue-exception-controls" data-queue-exception-controls>' +
    ...
    '<div class="queue-exception-actions' + controlClass + '">' + first + second + '</div>' +
  '</section>';
}
...
function openQueueModal(root, state, modal) {
  ...
  if (modal.type === "start") {
    ...
    var registrationModalMap = {
      open: { title: "팬 사전 접수를 지금 시작할까요?", copy: "팬 화면의 접수하기 버튼이 바로 활성화됩니다.", summary: "<strong>예정 시간 전에도 접수가 시작됩니다.</strong><span>특전회 시작 전까지 사전 접수를 받습니다.</span>", confirm: "지금 사전 접수 열기" },
      ...
      resume: { title: "팬 사전 접수를 다시 받을까요?", copy: "일시정지했던 신규 접수를 다시 활성화합니다.", summary: "<strong>팬의 접수하기 버튼이 다시 활성화됩니다.</strong><span>특전회 시작 전까지 사전 접수를 다시 받습니다.</span>", confirm: "새 접수 다시 열기" },
