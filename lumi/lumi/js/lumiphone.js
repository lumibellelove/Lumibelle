    ...
    (() => {
      ...
      function normalizeEntryStatusValue(value) {
        ...
        if (key === "checkedin" || key === "입장완료" || key === "입장완료처리") return "checkedIn";
        if (key === "notentered" || key === "미입장" || key === "대기") return "notEntered";
        return value || "";
      }
      ...
      function normalizeReservationItem(item) {
        ...
        const entryStatus = normalizeEntryStatusValue(source.entryStatus || source.entry || "");
        const reservationStatus = entryStatus === "checkedIn"
          ? "checkedIn"
          : (source.reservationStatus || "reserved");
        return {
      ...
      function reservationTicketHtml(item) {
        ...
        return '<article class="ticket-card lumi-pass">' +
          ...
          '<div class="ticket-cell"><small>ENTRY</small><b>' + escapeHtml(reservationStatusLabel(item.reservationStatus)) + '</b></div>' +
      ...
      function pcCurrentTicketHtml(item) {
        ...
        return '<h3>현재 예약 티켓</h3>' +
          ...
          '<div><small>ENTRY</small><b>' + escapeHtml(reservationStatusLabel(item.reservationStatus)) + '</b></div>' +
      ...
      function updateHomeReservationSummary(reservations) {
        ...
        if (ticketCard) {
          ...
          if (item) {
            if (small) small.textContent = item.reservationStatus === "checkedIn" ? "입장 완료" : paymentLabel(item.paymentStatus);
        ...
        if (summaryCard) {
          ...
          if (item) {
            ...
            if (desc) desc.textContent = (item.venueName || "공연장 확인 중") + " · " + (item.reservationStatus === "checkedIn" ? "입장 완료" : paymentLabel(item.paymentStatus));
          } else {
