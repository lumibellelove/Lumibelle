    ...
    (() => {
      ...
      function normalizeMemberFirstChekiChoices_(source) {
        return parseReplyOptionsFromSource_(source)
          .map(compactMemberFirstChekiChoice_)
          .filter(Boolean)
  ...
  function updateBadges(){
    ...
    if (homeCard) {
      ...
      if (first) {
        ...
        // fix2L-3-6U-fix15: unread가 0이어도 홈 문자 카드 자체는 유지한다.
        // 2026-05-12 5시39분 백업 기준 문구로 복원.
        homeCard.classList.remove("hidden");
        setHomeRoute_("message", "문자 보기");
        if (homeKicker) homeKicker.textContent = "MESSAGE";
        if (homeTitle) homeTitle.textContent = "문자함";
...
(function () {
  ...
  var ACH = [
    ...
    {id:'first-saved-message',cat:'online',state:'locked',icon:'💌',name:'처음 간직한 마음',desc:'처음으로 루미폰에 소중한 메시지를 간직한 기록이에요.',progress:'미달성',reward:'칭호 「마음을 간직한 루미나」',title:'마음을 간직한 루미나',date:'미달성',rule:'문자를 처음 소장하면 달성되는 문자/우편 업적이에요.'},
    ...
    {id:'first-cheki-mariring',cat:'field',state:'locked',icon:'🎀⭐️',name:'마리링 첫 특전',desc:'처음으로 마리링과 반짝이는 시간을 남긴 기록이에요.',progress:'0 / 1',reward:'칭호 「별빛을 따라간 자」',title:'별빛을 따라간 자',date:'미달성',rule:'마리링에게 특전권을 처음 사용하면 달성되는 멤버별 첫 특전 업적이에요.'},
    {id:'first-cheki-lulu',cat:'field',state:'locked',icon:'🍼🐰',name:'루루 첫 특전',desc:'처음으로 루루와 포근한 시간을 남긴 기록이에요.',progress:'0 / 1',reward:'칭호 「포근한 첫걸음」',title:'포근한 첫걸음',date:'미달성',rule:'루루에게 특전권을 처음 사용하면 달성되는 멤버별 첫 특전 업적이에요.'},
  ...
  function pcAchievementIdForApi(item) {
    ...
    var byKey = {
      ...
      first_saved_message: 'first-saved-message',
      first_checkin: 'first-checkin',
      first_cheki_mariring: 'first-cheki-mariring',
      member_first_cheki_mariring: 'first-cheki-mariring',
      first_cheki_lulu: 'first-cheki-lulu',
      member_first_cheki_lulu: 'first-cheki-lulu',
      stamp_1: 'stamp-one',
      stamp_5: 'stamp-five',
      stamp_10: 'stamp-ten',
    ...
    var byTitle = {
      ...
      '처음 간직한 마음': 'first-saved-message',
      '첫 루미 체크인': 'first-checkin',
      '마리링 첫 특전': 'first-cheki-mariring',
      '루루 첫 특전': 'first-cheki-lulu',
      '첫 번째 꽃도장': 'stamp-one',
      '스탬프 첫 장': 'stamp-one',
      '스탬프 5개': 'stamp-five',
      '스탬프 10개': 'stamp-ten',
